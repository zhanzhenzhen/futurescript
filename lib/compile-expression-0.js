...
export class ObjectExpression extends Expression {
    ...
    static applyMatch(match, lexPart, parentBlock) {
        return new this(
            new $statement.Arr(match.map(arg => {
                let membersMatch = $pattern.Pattern.split(
                    $lex.Colon,
                    lexPart.lex.part(arg.startIndex, arg.endIndex)
                );
                return new $statement.NameValue(
                    this.build(
                        lexPart.lex.part(membersMatch[0].startIndex, membersMatch[0].endIndex),
                        parentBlock
                    ),
                    this.build(
                        lexPart.lex.part(membersMatch[1].startIndex, membersMatch[1].endIndex),
                        parentBlock
                    )
                );
            }))
