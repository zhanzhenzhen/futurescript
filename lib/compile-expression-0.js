...
export class Expression {
    ...
    static build(lex, startIndex, endIndex, parentBlock) {
        let expression = (lex, startIndex, endIndex, parentBlock) => {
            if (startIndex === endIndex) {
                if (lex[startIndex] instanceof $lex.NormalToken) {
                    ...
                    if (match !== null) {
                        if (nearest === null) {
                            ...
                            if (group.leftToRight) {
                                if (match < Math.min(...nearest.match)) {
                                    nearest = {match: match, type: type};
                                }
                            }
                            else {
                                if (match > Math.max(...nearest.match)) {
                                    nearest = {match: match, type: type};
                                }
                            }
                        }
    ...
    static search(token, lex, startIndex, endIndex) {
        ...
        for (
            ...
            if (
                lex[i] instanceof $lex.NormalLeftParenthesis ||
                lex[i] instanceof $lex.CallLeftParenthesis
            ) {
                parenthesisLevel++;
            }
            else if (
                lex[i] instanceof $lex.RightParenthesis
            ) {
                parenthesisLevel--;
            }
            else if (lex[i] instanceof token && parenthesisLevel === 0) {
                return i;
            }
        }
        return null;
...
export class BinaryExpression extends Expression {
    ...
    static applyMatch(match, f, lex, startIndex, endIndex, parentBlock) {
        return new this(
            f(lex, startIndex, match[0] - 1, parentBlock),
            f(lex, match[0] + 1, endIndex, parentBlock)
        );
    }
};
...
export class IfExpression extends Expression {
    ...
    static match(lex, startIndex, endIndex, parentBlock) {
        ...
        if (thenPos !== null && thenPos > ifPos) {
            elsePos = this.search($lex.Else, lex, startIndex, endIndex);
        }
        if (elsePos !== null && elsePos > thenPos) {
            return [ifPos, thenPos, elsePos];
        }
        else {
            return null;
        }
