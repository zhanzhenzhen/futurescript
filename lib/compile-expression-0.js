...
export class BinaryExpression extends Expression {
    ...
    static applyMatch(match, lexPart, parentBlock) {
        let f = this.build;
        return new this(
            f({lex: lexPart.lex, startIndex: match[0].startIndex, endIndex: match[0].endIndex}, parentBlock),
            f({lex: lexPart.lex, startIndex: match[1].startIndex, endIndex: match[1].endIndex}, parentBlock)
        );
    }
};
...
export class ParenthesisCallExpression extends Expression {
    ...
    static match(lexPart, parentBlock) {
        ...
        let patternMatch = $pattern.Pattern.searchPattern(
            [$pattern.Tokens, $lex.CallLeftParenthesis, $pattern.Tokens, $lex.RightParenthesis],
            lexPart,
            false
        );
        if (patternMatch !== null) {
            return [
                {startIndex: startIndex, endIndex: patternMatch[1] - 1},
                {startIndex: patternMatch[2], endIndex: endIndex - 1}
            ];
        }
        else {
            return null;
    ...
    print(level = 0) {
        ...
        this.arguments.map(arg => arg.print(level + 1)).join("") +
        "    ".repeat(level + 1) + "]\n" +
        "    ".repeat(level) + "}\n";
    }
};
...
export class IfExpression extends Expression {
    ...
    static applyMatch(match, lexPart, parentBlock) {
        ...
        let thenStuff = null;
        if (match[1] !== null) {
            let thenLexPart = {lex: lex, startIndex: match[1].startIndex, endIndex: match[1].endIndex};
            thenStuff =
                lex.at(match[1].startIndex) instanceof $lex.LeftChevron ?
                new $block.Block(thenLexPart) :
                f(thenLexPart, parentBlock);
        }
        let elseStuff = null;
        if (match[2] !== null) {
            let elseLexPart = {lex: lex, startIndex: match[2].startIndex, endIndex: match[2].endIndex};
            elseStuff =
                lex.at(match[2].startIndex) instanceof $lex.LeftChevron ?
                new $block.Block(elseLexPart) :
                f(elseLexPart, parentBlock);
        }
        return new this(
            f({lex: lexPart.lex, startIndex: match[0].startIndex, endIndex: match[0].endIndex}, parentBlock),
            thenStuff,
