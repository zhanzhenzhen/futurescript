...
export class ArrowFunctionExpression extends FunctionExpression {
    ...
    static applyMatch(match, lex) {
        ...
        if (lex.at(match[0].startIndex) instanceof $lex.NormalLeftBracket) {
            if (lex.at(match[0].startIndex + 1) instanceof $lex.NormalLeftParenthesis) {
                argumentsLexPart = lex.part(match[0]).shrink().shrink();
                // So `argumentsReal` remains null, which means "possible real".
            }
            else {
    ...
    bareCompile() {
        ...
        if (this.argumentsReal === false) { // Why use false? Because can't include null.
            variables = new J(["[", variables, "]"]);
        }
        let assignVariables = this.argumentsReal === null ? new J([
            "if (Array.isArray(arguments[0])){",
            "[", new J(args.map(m => m.variable.compile()), ","), "]",
            "=arguments[0];}"
        ]) : "";
        ...
        if (
            ...
            if (this.getParent().x.static && this.getParent().x.name === null) {
                return [
                    ...
                    assignVariables, voidDefaults, nullDefaults,
