...
export class AssignStatement extends Statement {
    ...
    static build(lex, startIndex, endIndex, parentBlock) {
        ...
        if (lex.at(startIndex) instanceof $lex.Indent) {
            startIndex++;
        }
        if (lex.at(startIndex) instanceof $lex.NormalToken && lex.at(startIndex + 1) instanceof $lex.Colon) {
            return new AssignStatement(
                lex.at(startIndex).value,
                $expression.Expression.build(lex, startIndex + 2, endIndex, parentBlock)
            );
        }
        else {
            return null;
