...
export class Statement {
    static match(lexPart, parentBlock) {
        ...
        for (let i = 0; i < pcs.length; i++) {
            ...
            // For statement, `leftToRight` argument doesn't have any effect, but it's better
            // to set it to true so that it will run faster.
    ...
    static matchAndApply(lexPart, parentBlock) {
        ...
        if (match === null) {
            ...
            return this.applyMatch(match, lexPart, parentBlock);
        }
    }
...
export class AssignStatement extends Statement {
    ...
    static applyMatch(match, lexPart, parentBlock) {
        return new this(
            $expression.Expression.build(lexPart.changeTo(match[0]), parentBlock),
            $expression.Expression.build(lexPart.changeTo(match[1]), parentBlock)
        );
    }
