...
export class Statement {
    static buildBlock(match, lex, parentBlock) {
        let lexPart = lex.part(match);
        if (lex.at(match.startIndex) instanceof $lex.LeftChevron) {
...
export class AssignStatement extends Statement {
    ...
    static build(lexPart, parentBlock) {
        ...
        let match = $pattern.Pattern.matchPattern(
        ...
        if (match !== null) {
            return new AssignStatement(
                $expression.Expression.build(lex.part(startIndex, match[1] - 1), parentBlock),
                $expression.Expression.build(lex.part(match[2], endIndex), parentBlock)
            );
        }
        else {
            return null;
