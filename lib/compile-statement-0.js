...
export class AssignStatement extends Statement {
                ...
                if (dotMatch !== null) {
                    return new $node.DotAssignee({
                        ...
                        y: (
                            ...
                            lex.at(dotMatch[1].startIndex).value
                        ),
                        export: exportOn,
                        ifvoid: ifvoidOn,
                        ifnull: ifnullOn
                    ...
                    if (bracketMatch !== null) {
                        ...
                        return new $node.VariableAssignee({
                            variable: lex.at(startIndex).value,
                            export: exportOn,
                            ifvoid: ifvoidOn,
                            ifnull: ifnullOn
                        });
    ...
    compile() {
        let left = this.assignees.value.map(assignee => {
            if (assignee instanceof $node.BracketAssignee) {
                return "[" + assignee.elements.value.map(m => m.value).join(",") + "]";
            }
            else if (assignee instanceof $node.DotAssignee) {
                if (assignee.y instanceof $expression.Expression) {
                    return assignee.x.compile() + "[" + assignee.y.compile() + "]";
                }
                else {
                    return assignee.x.compile() + "." + assignee.y.value;
                }
            }
            else {
                return assignee.variable.value;
            }
        }).join("=");
        return left + "=" + this.value.compile();
    }
}
