    ...
    rawCompile() {
        ...
        if (this.items.value.length === 0) {
            ...
            let items = treeNonElseItems.length === 0 ? new J("false?undefined") : new J(
                treeNonElseItems.map(m => new J([
                    (
                        $tools.instanceof(this.comparer, [
                            $expressionFunction.ArrowFunctionExpression,
                            $expressionFunction.DiamondFunctionExpression,
                            $expressionFunction.DashFunctionExpression
                        ]) ?
                        new J([comparer + "(", m.x.compile(), ")"]) :
                        new J([comparer + "===", m.x.compile()])
                    ),
                    "?",
                    m.y.compile()
        ...
        return ["function(){", comparerDeclaration, "return ", body, ";}()"];
    }
}
