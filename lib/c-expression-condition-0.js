...
pool.IfExpression = class IfExpression extends pool.Expression {
...
pool.PostIfExpression = class PostIfExpression extends pool.Expression {
    static patternsAndCaptures() {
        return [
...
pool.MatchExpression = class MatchExpression extends pool.Expression {
        ...
        let items = new $node.Arr(itemRanges.map(m => {
            ...
            if (pair[0] !== null) {
                ...
                if (hasElse) {
                    ...
                    additionalElseToAdd = $statement.Statement.buildBlock(lex.part(pair[1]));
                }
        ...
        if (this.items.value.length === 0) {
            ...
            let conditionPart = x =>
                $tools.instanceof(this.comparer, [
                    pool.ArrowFunctionExpression,
                    pool.DiamondFunctionExpression,
                    pool.DashFunctionExpression
                ]) ?
