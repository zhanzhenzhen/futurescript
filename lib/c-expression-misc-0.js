...
pool.AsExpression = class AsExpression extends pool.Expression {
        ...
        if (dotMatch !== null) {
            assignee = new $node.DotAssignee({
                ...
                y: (
                    ...
                    this.build(lex.part(dotMatch[1])) :
    ...
    rawCompile() {
        ...
        if (this.assignee instanceof $node.DotAssignee) {
            if (this.assignee.y instanceof pool.Expression) {
...
pool.IfvoidExpression = class IfvoidExpression extends pool.BinaryExpression {
    rawCompile() {
        this.getRoot().predefinedLib.ifvoid = true;
        return ["ifvoid_" + $block.antiConflictString + "(", this.x.compile(), ",", this.y.compile(), ")"];
    }
}
pool.IfvoidExpression.sign = $lex.Ifvoid;

pool.IfnullExpression = class IfnullExpression extends pool.BinaryExpression {
    rawCompile() {
        this.getRoot().predefinedLib.ifnull = true;
        return ["ifnull_" + $block.antiConflictString + "(", this.x.compile(), ",", this.y.compile(), ")"];
    }
}
pool.IfnullExpression.sign = $lex.Ifnull;
