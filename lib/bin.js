...
let defaultTargetRootDir = $path.join(moduleDir, "../target");
...
let makeSingleVersion = (version, targetDir) => {
    ...
    let sourceDir = $path.join(moduleDir, dirname);
    ...
    refList.forEach(item => {
        let sourcePath = $path.join(moduleDir, "c-v" + item[0], item[1]);
...
let makeAllVersions = targetDir => {
    $fs.readdirSync(moduleDir).filter(m => m.startsWith("c-v")).forEach(dirname => {
...
if (args[0] === "compile" || args[0] === "c") {
    ...
    let libDir = $path.join(moduleDir, "../lib");
    ...
    if (args[0] === "c-current" || args[0] === "cc") {
        ...
        rm.sync(defaultTargetRootDir);
        makeSingleVersion(cwdVersion, $path.join(defaultTargetRootDir, "c-v" + cwdVersion));
        process.stdout.write($cp.execFileSync(
            process.execPath,
            [
