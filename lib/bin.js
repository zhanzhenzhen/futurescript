...
let compile = (arg1, arg2, maps = false) => {
    ...
    let write = (targetPath, target) => {
        $fs.writeFileSync(targetPath, target.code);
        if (maps) {
            $fs.writeFileSync(targetPath + ".map", target.sourceMap);
        }
    };
    ...
    if (stat.isDirectory()) {
        if (arg2 === undefined) {
            $fs.readdirSync(arg1).forEach(item => {
                ...
                if (!$fs.statSync(item).isFile()) return;
                if ($path.extname(path) === ".lemo") {
                    let output = $lockedApi.generateOutput({path: path, level: level});
        ...
        let output = $lockedApi.generateOutput({path: arg1, level: level});
        let targetPath = arg2;
