...
export class ArrowFunctionExpression extends $expressionBase.Expression {
            ...
            new $node.Arr(argumentRanges.map(m => {
                if (lex.at(m.startIndex + 1) instanceof $lex.Ifvoid) {
                    return new $node.ArrowArgument({
                        variable: lex.at(m.startIndex).value,
    ...
    rawCompile() {
        ...
        let variables = args.map(m => m.variable).join(",");
        let voidDefaults = args.filter(m => m.voidDefault).map(m =>
            "if (" + m.variable + "===undefined" + "){" +
            m.variable + "=" + m.voidDefault.compile() + ";}"
        ).join("");
        let nullDefaults = args.filter(m => m.nullDefault).map(m =>
            "if (" + m.variable + "===null" + "){" +
            m.variable + "=" + m.nullDefault.compile() + ";}"
        ).join("");
...
export class ParenthesisCallExpression extends $expressionBase.Expression {
    ...
    rawCompile() {
        let args = this.arguments.value.map(m => m.compile()).join(",");
