...
export class Pattern {
    // `ruler` can't be a pair or quantifier.
    static searchOne(ruler, lexPart, leftToRight) {
        if ($tools.classIsClass(ruler, Pair) || ruler instanceof Quantifier) {
            throw new Error();
        }
    ...
    Quantifiers can't be consecutive.
    ...
    */
    static matchPattern(rulers, lexPart, leftToRight) {
        rulers.forEach((ruler, index) => {
            if (rulers[index - 1] instanceof Quantifier && ruler instanceof Quantifier) {
                throw new Error();
            }
        });
