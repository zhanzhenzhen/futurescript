...
let checkNonWildcard = (index, ruler, lexPart, leftToRight) => {
    let lex = lexPart.lex;
    let token = lex.at(index);
...
export class Pattern {
    ...
    static matchPattern(rulers, lexPart, leftToRight) {
        ...
        while (
            ...
            else if (checkNonWildcard(lexCursor, ruler, lexPart, leftToRight)) {
                patternCursor += leftToRight ? 1 : -1;
                lexCursor = sameLevelNextIndex(lexCursor, lex, leftToRight);
            }
            else {
                return null;
            }
