...
let sameLevelNext = (index, lex) => {
};

// If matched, returns a 2-element array representing the start and end index of the match
// (end index can be shorter than start, meaning right-to-left).
// If not matched, returns null.
let matchNonWildcard = (index, ruler, lexPart, leftToRight) => {
    let lex = lexPart.lex;
    ...
    if ($tools.classIsClass(ruler, Pair)) { // for jumping quickly if it isn't pair
        ...
        if (matched) {
            if (
                ...
                r = [index, token.oppositeIndex];
            }
        }
    }
    else if (checkToken(index, ruler, lexPart)) {
        r = [index, index];
    }
    return r;
};
...
export class Pattern {
    ...
    static matchPattern(rulers, lexPart, leftToRight) {
        ...
        let startIndex = lexPart.startIndex;
        let endIndex = lexPart.endIndex;
        let lexCursor = leftToRight ? startIndex : endIndex;
        let patternCursor = leftToRight ? 0 : rulers.length - 1;
        let result = [];
        let matchWildcardAfter = null;

        while (
            leftToRight ?
            lexCursor <= endIndex && patternCursor < rulers.length :
            lexCursor >= startIndex && patternCursor >= 0
        ) {
            let token = lex.at(lexCursor);
            let ruler = rulers[patternCursor];
            let matched = false;

            if (matchWildcardAfter !== null) {
                patternCursor += leftToRight ? 1 : -1;
                lexCursor = matchWildcardAfter[1] + (leftToRight ? 1 : -1);
                matched = true;
                matchWildcardAfter = null;
            }
            else if (ruler instanceof AnyExcept) {
                if (leftToRight ? patternCursor + 1 <= rulers.length - 1 : patternCursor - 1 >= 0) {
                    matchWildcardAfter = matchNonWildcard(
                        lexCursor, rulers[patternCursor + 1], lexPart, leftToRight
                    );
                }
                if (matchWildcardAfter !== null || $tools.instanceof(token, ruler.tokenTypes)) {
                    patternCursor += leftToRight ? 1 : -1;
                }
                else {
                    lexCursor = matchWildcardAfter[1] + (leftToRight ? 1 : -1);
                ...
                match = matchNonWildcard(lexCursor, ruler, lexPart, leftToRight);
                patternCursor += leftToRight ? 1 : -1;
                lexCursor = match[1] + (leftToRight ? 1 : -1);
            }

            result.push(lexCursor);
        }
    }
