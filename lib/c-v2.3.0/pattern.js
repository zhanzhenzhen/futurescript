...
let searchNonWildcard = (index, ruler, lexPart, leftToRight) => {
    let lex = lexPart.lex;
    let token = lex.at(index);
    let r = null;
    ...
    static matchPattern(rulers, lexPart, leftToRight) {
        ...
        while (
            ...
            if (wildcardAfter !== null) {
                ...
                lexCursor = sameLevelNextIndex(wildcardAfter, lex, leftToRight);
                wildcardAfter = null;
            }
            else if (ruler instanceof AnyExcept) {
                if (leftToRight ? patternCursor + 1 <= rulers.length - 1 : patternCursor - 1 >= 0) {
                    wildcardAfter = searchNonWildcard(
                        lexCursor, rulers[patternCursor + 1], lexPart, leftToRight
                    );
                }
                if (wildcardAfter !== null || $tools.instanceof(token, ruler.tokenTypes)) {
                    patternCursor += leftToRight ? 1 : -1;
                }
                else {
                    lexCursor = sameLevelNextIndex(wildcardAfter, lex, leftToRight);
                ...
                let index = searchNonWildcard(lexCursor, ruler, lexPart, leftToRight);
                if (index === null) {
                    return null;
                }
                else {
                    patternCursor += leftToRight ? 1 : -1;
                    lexCursor = sameLevelNextIndex(index, lex, leftToRight);
                }
            }
