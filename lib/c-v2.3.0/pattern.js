    ...
    static matchPattern(rulers, lexPart, leftToRight) {
        ...
        while (
            ...
            if (wildcardAfter !== null) {
                ...
                nextLexCursor = sameLevelNextIndex(wildcardAfter, lex, leftToRight);
                wildcardAfter = null;
            }
            else if (ruler instanceof Quantifier) {
                if (wildcardTokenCount === null) {
                    wildcardTokenCount = 0;
                }
                ...
                if (
                    patternCursor === (leftToRight ? rulers.length - 1 : 0) ||
                    wildcardAfter !== null ||
                    $tools.instanceof(token, ruler.tokenTypes)
                ) {
                    if (ruler instanceof TokensExcept && wildcardTokenCount === 0) {
                        return null;
                    }
                    wildcardTokenCount = null;
                    nextPatternCursor += leftToRight ? 1 : -1;
                }
                else {
                    wildcardTokenCount++;
            ...
            if (patternCursor !== nextPatternCursor) {
