...
// Merge two sorted arrays into one sorted array. The two arrays must be sorted by key
// (with `keySelector` function) in ascending order in advance.
// `arrays` is an array of the two arrays.
export let mergeSorted = function(arrays, keySelector) {
    let a = arrays[0];
    let b = arrays[1];
    let i = 0;
    let j = 0;
    ...
    while (i < a.length && j < b.length) {
        if (keySelector(a[i]) <= keySelector(b[j])) {
            r.push(a[i]);
            i++;
        }
        else {
