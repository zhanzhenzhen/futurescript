...
import * as $node from "../c-v0.2.0/node.js";
import * as $nodeBase from "../c-v0.1.0/node-base.js";
import * as $print from "../c-v0.1.0/print.js";
import * as $lockedApi from "../locked-api.js";
import * as $tools from "../c-v0.1.0/tools.js";
...
export class Block extends $nodeBase.MappableNode {
    ...
    compile() {
        let jb = new J(this.value.map((statement, index) => {
            ...
            let lex = lexPart.lex;
            let token = lex.at(lexPart.startIndex);
            let commentMap = "";
            if (root.outputCodeReadability === 1 && token.rawStartIndex !== undefined) {
                commentMap = "/*" + lex.rawLineColumn(token.rawStartIndex) + "*/";
            }
            if (
                ...
                return new J(["return ", statement.compile(), ";" + commentMap]);
            }
            else {
                return new J([statement.compile(), ";" + commentMap]);
            }
        }));
