...
pool.ClassExpression = class ClassExpression extends pool.Expression {
    ...
    rawCompile() {
        ...
        this.body.value.forEach(m => {
            if (m.x.static && m.x.name === null) {
                ...
                m.y instanceof pool.ArrowFunctionExpression ||
                m.y instanceof pool.DiamondFunctionExpression ||
                m.y instanceof pool.DashFunctionExpression
            ) {
        ...
        let inner = new J(treeFunctionMembers.map(m => {
            ...
            return new J([
                ...
                (m.x.get ? "get " : "") +
                (m.x.set ? "set " : ""),
                (m.x.new ? "constructor" : m.x.name.compile()),
                "(", variables, "){var ", funName,
                ";" + (m.x.new ? "" : "return ") + "(", funName, "=", m.y.compile(), ")(", variables, ");}"
            ]);
        }));
        let part1 = this.getSymbolMemberNames().map(m => "var " + m + "=Symbol();").join("");
        let part2 = new J(["var " + classVar + "=class {", inner, "};"]);
        ...
        return ["(()=>{", part1, part2, part3, part4, "return " + classVar + ";})()"];
    }
