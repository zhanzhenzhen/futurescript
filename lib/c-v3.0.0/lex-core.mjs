...
// Note: this doesn't apply to a token before `:` or after `as`. For example,
// these are legal:
// ..: something
// something as ..
...
export class Lex {
    ...
    constructor(raw, path) {
        ...
        if (raw === undefined) {
            return new Promise((resolve, reject) => {(async () => {try {
    ...
    _round1() {
        ...
        while (i < raw.length) {
            ...
            if (pendingToken instanceof VersionDirective) {
                ...
                if (!numberIsNotDecimal && char === "e" && (raw[i + 1] === "+" || raw[i + 1] === "-")) {
                    pendingToken.value += "e" + raw[i + 1];
                    i += 2;
                }
                else if ((
                    numberForcesInt ?
                    char.search(/[0-9A-Fa-fxo]/) :
                    char.search(/[0-9A-Fa-fxo.]/)
                ) !== -1) {
                    pendingToken.value += char;
                    i++;
                ...
                if (postQuote !== null) {
                    ...
                    // `quoteEscapeEnabled` also means "\\" escape is enabled
            ...
            // recognize length-fixed tokens: double character
                ...
                if (raw[i + 2].search(/[A-Za-z_$]/) !== -1) {
                    result.push(new DotDot().start(i).end(i + 1));
                }
                else {
                    // TODO: Anti-conflict string should be in a more general module so that the lex
                    // module can also use it.
                    result.push(new NormalToken("dotDot_573300145710716007").start(i).end(i + 1));
                }
                i += 2;
            }
            else if (char === "/" && raw[i + 1] === "=") {
            ...
            // recognize length-unfixed tokens: single character
                ...
                if (char === "0" && (raw[i + 1] === "x" || raw[i + 1] === "b" || raw[i + 1] === "o")) {
                    numberIsNotDecimal = true;
                }
                i++;
            }
            else if (char === "\"") {
...
export class LexPart {
    // `endIndex` can be smaller than `startIndex` by 1, representing an empty part.
    ...
    // Note that if the lex part is empty, it still returns the token at `startIndex`.
    token() {
...
export class MatchNoBodyError extends SyntaxError {
    constructor(lexPart) {
        super(lexPart, "`match` must have body represented by indents (or `<<` and `>>`).");
    }
}
...
export class Enclosing extends Token {}
export class Chevron extends Enclosing {}
export class LeftChevron extends Chevron {} LeftChevron.leftFlag = true;
export class RightChevron extends Chevron {}
export class Parenthesis extends Enclosing {}
export class LeftParenthesis extends Parenthesis {} LeftParenthesis.leftFlag = true;
...
export class Bracket extends Enclosing {}
export class LeftBracket extends Bracket {} LeftBracket.leftFlag = true;
...
export class Brace extends Enclosing {}
export class LeftBrace extends Brace {} LeftBrace.leftFlag = true;
...
export class DotDot extends Token {} DotDot.isJoint = true;
