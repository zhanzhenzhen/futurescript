...
export class MatchExpression extends $expressionBase.Expression {
    ...
    static applyMatch(match, lex, parentBlock) {
        let comparisonLastToken = lex.at(match[0].endIndex);
        let comparison = null;
        if (
            comparisonLastToken instanceof $lex.Equal ||
            comparisonLastToken instanceof $lex.NotEqual ||
            comparisonLastToken instanceof $lex.LessThan ||
            comparisonLastToken instanceof $lex.GreaterThan ||
            comparisonLastToken instanceof $lex.LessThanOrEqual ||
            comparisonLastToken instanceof $lex.GreaterThanOrEqual
        ) {
        }
        else {
            comparison = this.build(lex.part(match[0]), parentBlock);
        }
        let itemRanges = $pattern.Pattern
        .split($lex.Semicolon, lex.part(match[1]).shrink())
        .filter(m => m.endIndex >= m.startIndex);
        let items = itemRanges.map(m => {
            $pattern.Pattern.matchPatternsAndCaptures([
                [[$pattern.tokensExcept([$lex.LeftChevron]), $lex.Then, $pattern.any], [1, 3, null]]
            ], lexPart, true);
            return this.build(lex.part(m), parentBlock);
        });
        return new this(
            this.build(lex.part(match[0]), parentBlock),
            $statement.Statement.buildBlock(lex.part(match[1]), parentBlock),
