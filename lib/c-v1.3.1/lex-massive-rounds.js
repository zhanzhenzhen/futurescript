...
Lex.prototype._round9 = function() {
    ...
    while (i < this.value.length) {
        ...
        let nextNext = this.value[i + 2];
        if (
            token instanceof $lex.NormalLeftParenthesis &&
            next instanceof $lex.DotDot &&
            nextNext instanceof $lex.NormalRightParenthesis
        ) {
            newValue.push(new $lex.NormalToken("dotDot_573300145710716007")
            .start(i).end(i + 2).setOriginalTokens([token, next, nextNext]));
            i += 3;
        }
        else if (token instanceof $lex.Export && next instanceof $lex.As) {
