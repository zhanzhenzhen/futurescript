...
export class Lex {
    ...
    */
    ...
    // Two features: Mark levels, and correct the default export token.
    _round22() {
        let newValue = [];
        let level = 0;
        this.value.forEach((token, index) => {
...
export class Token {
    ...
    // Create a new instance with the same type, same position, same style,
    // but different value, returning the new instance.
    replaceWith(value) {
        return new this.constructor(value).copyPositionFrom(this).addStyle(this.style);
    }
