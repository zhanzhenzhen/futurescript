...
Lex.prototype._round9 = function() {
    let newValue = [];
    this.value.forEach((token, index) => {
        if (token instanceof $lex.DotDot && (
            this.value[index + 1] instanceof $lex.Colon ||
            (
                this.value[index + 1] instanceof $lex.NormalVariant &&
                this.value[index + 3] instanceof $lex.Colon
            ) ||
            this.value[index - 1] instanceof $lex.As
        )) {
            // TODO: Anti-conflict string should be in a more general module so that the lex
            // module can also use it.
            newValue.push(new $lex.NormalToken("dotDot_573300145710716007").copyPositionFrom(token));
        }
        else {
            newValue.push(token);
        }
...
Lex.prototype._round33 = function() {
    let stack = [];
    this.value.forEach((token, index) => {
        if (
            token instanceof $lex.LeftChevron ||
