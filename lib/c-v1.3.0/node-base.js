...
// the source and the compiled code look the same (unless renamed to avoid conflict or invalidity).
export class Piece extends MappableNode {
    constructor() {
        ...
        if (typeof arg === "string") {
            ...
            else if (arg.token() instanceof $lex.DotDot) {
                this.value = "dotDot_573300145710716007";
            }
            else {
...
export let compileAssignee = (assignee) => {
    ...
    if (assignee instanceof $node.BracketAssignees) {
        ...
        if (
            ...
            second = new J([assignee.x.compile(), "[", assignee.y.compile(), "]"]);
        }
        else {
            second = new J([assignee.x.compile(), ".", assignee.y.compile()]);
        }
