...
$api.writeTextFile($api.tempDirectory + "/c.fus", code`, node modules
import "./a.fus" all
`);
lex = await new $lex.Lex(undefined, $api.tempDirectory + "/c.fus");
...
$api.writeTextFile($api.tempDirectory + "/d.fus", code`, node modules
import "./a.mjs" all
`);
lex = await new $lex.Lex(undefined, $api.tempDirectory + "/d.fus");
block = await new $node.RootBlock(lex);
assert(block.toString() === String.raw`node modules
RootBlock [
    ImportStatement {
