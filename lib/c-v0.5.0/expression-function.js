...
export class ParenthesisCallExpression extends $base.Expression {
    ...
    static applyMatch(match, lex) {
        let isNew = false;
        let isNonew = false;
        if ($pattern.Pattern.matchPattern([$lex.New, $pattern.tokensExcept([
            $lex.LeftParenthesis,
            $lex.LeftBracket,
            $lex.LeftBrace
        ])], lex.part(match[0]), true) !== null) {
            match[0].startIndex++;
            isNew = true;
        }
        else if ($pattern.Pattern.matchPattern([$lex.Nonew, $pattern.tokensExcept([
            $lex.LeftParenthesis,
            $lex.LeftBracket,
            $lex.LeftBrace
        ])], lex.part(match[0]), true) !== null) {
