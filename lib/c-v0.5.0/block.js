    ...
    return function() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            return new J([
                ...
                "\"use strict\";",
                hoisted,
                libMod, libOk, libIfvoid, libIfnull, libFormattedRegex, libAsyncToGenerator,
                jb,
                lowered
            ]);
        }
            ...
            if (this.getParent() instanceof $node.FunctionExpression && this.getParent().getAsync()) {
                return new J([`(asyncToGenerator_${antiConflictString}(function*(){`, jb, "})())"]);
            }
            else {
                return new J(["((()=>{", jb, "})())"]);
            }
    ...
    constructor(lex, outputCodeReadability = 0) {
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $node.WaitExpression) {
                node.getParent($node.FunctionExpression).setAsync(true);
            }
        });
