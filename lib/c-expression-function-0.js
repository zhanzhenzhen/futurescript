...
export class ArrowFunctionExpression extends $expressionBase.Expression {
            ...
            new $node.Arr(argumentRanges.map(m => {
                if (lex.at(m.startIndex + 1) instanceof $lex.Ifvoid) {
                    return new $node.ArrowArgument({
                        ...
                        export: false,
                        ifvoid: this.build(lex.part(m.startIndex + 3, m.endIndex)),
                        ifnull: null
                    });
                }
                    ...
                    return new $node.ArrowArgument({
                        variable: new $node.Piece(lex.part(m)),
                        export: false,
                        ifvoid: this.build(lex.part(m.startIndex + 2, m.endIndex)),
                        ifnull: null
                    });
                }
                    ...
                    return new $node.ArrowArgument({
                        ...
                        export: false,
                        ifvoid: null,
                        ifnull: this.build(lex.part(m.startIndex + 3, m.endIndex))
                    });
                }
                else {
                    return new $node.ArrowArgument({
    ...
    rawCompile() {
        ...
        let voidDefaults = new J(args.filter(m => m.ifvoid !== null).map(m =>
            new J([
                "if (",
                m.variable.compile(),
                "===undefined){",
        ...
        let nullDefaults = new J(args.filter(m => m.ifnull !== null).map(m =>
            new J([
                "if (",
                m.variable.compile(),
                "===null){",
...
export class PseudoCallExpression extends $expressionBase.Expression {
    constructor(callee, args) {
        super();
        this.callee = callee;
        this.arguments = args;
