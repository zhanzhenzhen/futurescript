...
export class ArrowFunctionExpression extends pool.Expression {
...
export class DiamondFunctionExpression extends pool.Expression {
    constructor(body) {
        super();
        this.body = body;
    }
...
export class DashFunctionExpression extends pool.Expression {
    constructor(body) {
        super();
        this.body = body;
    }
...
export class ParenthesisCallExpression extends pool.Expression {
    ...
    static applyMatch(match, lex) {
        let isNew = false;
        let isNonew = false;
        if (lex.at(match[0].startIndex) instanceof $lex.New) {
            match[0].startIndex++;
            isNew = true;
        }
        else if (lex.at(match[0].startIndex) instanceof $lex.Nonew) {
            match[0].startIndex++;
            isNonew = true;
        }
        let argumentRanges = $pattern.Pattern
    ...
    rawCompile() {
        let jb = (callee) => {
            ...
            if (root.hasCompilerDirective("manual new")) {
                ...
                if (this.new) {
                ...
                else if (this.callee instanceof pool.VariableExpression) {
                    if (this.callee.value[0] >= "A" && this.callee.value[0] <= "Z") {
                        useNew = true;
                    }
                    else {
                        useNew = false;
                    ...
                    this.callee instanceof pool.DotExpression &&
...
export class BracketCallExpression extends pool.Expression {
    static patternsAndCaptures() {
        return [
...
export class BraceCallExpression extends pool.Expression {
    static patternsAndCaptures() {
        return [
...
export class SpaceCallExpression extends pool.Expression {
...
export class PseudoCallExpression extends pool.Expression {
...
export class DoExpression extends pool.Expression {
    constructor(f) {
        super();
        this.function = f;
    }
