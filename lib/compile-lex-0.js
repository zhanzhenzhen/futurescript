...
export class Lex {
    ...
    */
        ...
        this.value.forEach((token, index) => {
            if (token instanceof LeftChevron) {
                if (newValue[newValue.length - 1] instanceof Join) {
                    ...
                    this.value[index - 1].constructor.isJoint ||
                    this.value[index + 1].constructor.isJoint
                ) {
                    stack.push(true);
                }
    ...
    _round16() {
        let newValue = [];
        this.value.forEach((token, index) => {
            if (token instanceof Comma && this.value[index + 1] instanceof Semicolon) {
            }
            else {
                newValue.push(token);
            }
    ...
    part(startIndex, endIndex) {
        return new LexPart(this, startIndex, endIndex);
    }
...
export class LexPart {
    ...
    changeTo() {
        if (arguments[1] === undefined) {
            return new LexPart(this.lex, arguments[0].startIndex, arguments[0].endIndex);
        }
        else {
            return new LexPart(this.lex, arguments[0], arguments[1]);
        }
    }
}
...
export class Arg extends Token {}
...
export class Fun extends Token {}
...
export class ClassMe extends Token {}
...
export class Self extends Token {}
