    ...
    _round1(raw) {
        ...
        while (i < raw.length) {
            ...
            if (pendingToken instanceof VersionDirective) {
                ...
                if (!(char === "\"" && (
                    stringType.isSingleLine || (!stringType.isSingleLine && indentOffset === 0)
                ))) {
                    let interpolationChar = null;
                    if (stringType === SingleLineNormalString) {
                        interpolationChar = "\\";
                    }
                    else if (stringType === MultiLineNormalString) {
                        interpolationChar = "\\";
                    }
            ...
            // recognize length-unfixed tokens: double character
                ...
                if (raw[i + 2] === "\r" || raw[i + 2] === "\n") {
                    stringType = MultiLineVerbatimString;
                    result.push(new MultiLineVerbatimString());
                }
                else {
                    stringType = SingleLineVerbatimString;
                    result.push(new SingleLineVerbatimString());
                }
            ...
            // recognize length-unfixed tokens: single character
                ...
                if (raw[i + 1] === "\r" || raw[i + 1] === "\n") {
                    stringType = MultiLineNormalString;
                    result.push(new MultiLineNormalString());
                }
                else {
                    stringType = SingleLineNormalString;
                    result.push(new SingleLineNormalString());
                }
                result.push(new CallLeftParenthesis());
                pendingToken = new Str([]);
                pendingToken.type = stringType;
...
export class SingleLineNormalString extends Token {} SingleLineNormalString.isSingleLine = true;
export class MultiLineNormalString extends Token {} MultiLineNormalString.isSingleLine = false;
export class SingleLineVerbatimString extends Token {} SingleLineVerbatimString.isSingleLine = true;
export class MultiLineVerbatimString extends Token {} MultiLineVerbatimString.isSingleLine = false;
export class SingleLineRegex extends Token {} SingleLineRegex.isSingleLine = true;
export class MultiLineRegex extends Token {} MultiLineRegex.isSingleLine = false;
export class SingleLineJs extends Token {} SingleLineJs.isSingleLine = true;
export class MultiLineJs extends Token {} MultiLineJs.isSingleLine = false;
...
let stringEscapeChar = function(stringType) {
    ...
    else if (stringType === SingleLineVerbatimString) {
        return {
            "\"\"": "\""
        };
    }
