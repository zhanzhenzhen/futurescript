    ...
    */
    ...
    _round16() {
        ...
        this.value.forEach((token, index) => {
            ...
            let next = this.value[index + 1];
            if (
                (token instanceof RightParenthesis || token.canBeCalleeEnd) &&
                next !== undefined &&
                (
                    next instanceof NormalLeftParenthesis ||
                    next instanceof NormalLeftBracket ||
                    next instanceof NormalLeftBrace ||
                    !(next.expressionStartForbidden || next.isJoint)
                )
            ) {
                newValue.push(new SpaceCall());
            }
        });
        this.value = newValue;
    }
...
export class FunctionVariant extends Token {} FunctionVariant.canBeCalleeEnd = true;
