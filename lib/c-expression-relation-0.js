...
pool.InExpression = class extends pool.BinaryExpression {
    rawCompile() {
...
pool.NotInExpression = class extends pool.BinaryExpression {
    rawCompile() {
        return [this.y.compile(), ".indexOf(", this.x.compile(), ")<0"];
    }
}
...
pool.IsExpression = class extends pool.BinaryExpression {
    rawCompile() {
...
pool.IsntExpression = class extends pool.BinaryExpression {
    rawCompile() {
        if (this.y instanceof pool.VariableExpression && (
            this.y.value === "Number" ||
            this.y.value === "Boolean" ||
