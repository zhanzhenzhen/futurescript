...
export class InExpression extends $node.BinaryExpression {
    rawCompile() {
...
export class NotInExpression extends $node.BinaryExpression {
    rawCompile() {
        return [this.y.compile(), ".indexOf(", this.x.compile(), ")<0"];
    }
}
...
export class IsExpression extends $node.BinaryExpression {
    rawCompile() {
...
export class IsntExpression extends $node.BinaryExpression {
    rawCompile() {
        if (this.y instanceof $node.VariableExpression && (
            this.y.value === "Number" ||
            this.y.value === "Boolean" ||
