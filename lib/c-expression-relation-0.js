...
export class InExpression extends pool.BinaryExpression {
    rawCompile() {
...
export class NotInExpression extends pool.BinaryExpression {
    rawCompile() {
        return [this.y.compile(), ".indexOf(", this.x.compile(), ")<0"];
    }
}
...
export class IsExpression extends pool.BinaryExpression {
    rawCompile() {
...
export class IsntExpression extends pool.BinaryExpression {
    rawCompile() {
        if (this.y instanceof pool.VariableExpression && (
            this.y.value === "Number" ||
            this.y.value === "Boolean" ||
