...
// Note: In the test code, for `a` we use `setTimeout` with the timeout being zero, and
// for `b` we use non-async but we `wait` for it. This is safe, because in JavaScript,
// non-async promises are always executed before `setTimeout`, even if the timeout is zero,
// and even if there is a long promise chain in which all promises are non-async.
testEndless(async () => {
let r = await $api.runCodeEndless(code`
a: <>
    Promise((resolve, reject) ->
...
do --
    t:
        try
            result.push promise(1)'wait
            a()
        finally
            result.push promise(2)'wait
    result.push promise(3)'wait
    ended: true
`);
assert(
    r[0] === 1 &&
...
// Test the tricks in `match`. This is similar to the test
