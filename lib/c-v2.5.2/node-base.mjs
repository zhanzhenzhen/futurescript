...
// `compileWrapperFunction` may be ambiguous. It's better to be `compileToWrapperFunction`.
export let compileWrapperFunction = (innerJb, node, asyncTangle = null) => {
    if (node.getHasWaitBehavior()) {
        // Trick to avoid JS's bad feature of "folding" promises.
...
export let compileToReturn = (innerJb, container) => {
    if (
        !(container instanceof $node.Block && container.getParent() instanceof $node.FunctionExpression) &&
        container.getHasWaitBehavior()
    ) {
        /*
