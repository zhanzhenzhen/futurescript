...
export class Node {
    ...
    getAncestors(selector, includesMatched = true) {
        let next = this;
        let stack = [];
        while (true) {
            next = next._parent;
            ...
            let found =
                selector === undefined ?
                false :
                ($tools.classIsClass(selector, Node) ? next instanceof selector : selector(next));
...
export let compileToReturn = (innerJb, node) => {
    if (
