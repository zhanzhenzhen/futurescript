...
export class AssignStatement extends Statement {
    ...
    constructor(assignees, value) {
        ...
        this.setUseBase1(false);
        this.setUseBase2(false);
    }
    ...
    setUseBase1(useBase1) {
        this._useBase1 = useBase1;
    }

    getUseBase1() {
        return this._useBase1;
    }

    setUseBase2(useBase2) {
        this._useBase2 = useBase2;
    }

    getUseBase2() {
        return this._useBase2;
    }
    ...
    compile() {
        ...
        if (this.getUseBase1() || this.getUseBase2() || firstAssignee.ifvoid || firstAssignee.ifnull) {
            let left = $node.compileAssignee(firstAssignee);
            let jb = new J([left, "=", this.value.compile()]);
            let declaration1 = new J(
                this.getUseBase1() ?
                ["var base1_" + $node.antiConflictString + "=", firstAssignee.x.compile(), ";"] :
                ""
            );
            let declaration2 = new J(
                this.getUseBase2() ?
                ["var base2_" + $node.antiConflictString + "=", firstAssignee.y.compile(), ";"] :
                ""
            );
            if (firstAssignee.ifvoid) {
                jb = new J(["if (", $node.compileAssignee(firstAssignee), "===undefined){", jb, ";}"]);
            }
            else if (firstAssignee.ifnull) {
                jb = new J([
                    "if (",
            ...
            return new J([declaration1, declaration2, jb]);
        }
        else {
