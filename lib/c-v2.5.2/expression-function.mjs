    ...
    bareCompile() {
        ...
        if (
            ...
            if (this.getParent().x.static && this.getParent().x.name === null) {
                return [
                    "function" + (hasFun ? " fun_" + $node.antiConflictString : "") + "(", variables, "){",
                ...
                return [
                    hasFun ? "(()=>{var fun_" + $node.antiConflictString + "=" : "",
                    "(", variables, ")=>{",
                    voidDefaults, nullDefaults,
                    "return ", this.body.compile(), ";}",
                    hasFun ? ";return fun_" + $node.antiConflictString + ";})()" : ""
                ];
            }
        }
        else {
            if (this.getAncestor($node.ClassExpression) === null) {
                return [
                    "function" + (hasFun ? " fun_" + $node.antiConflictString : "") + "(", variables, "){",
                    voidDefaults, nullDefaults,
                    "return ", this.body.compile(), ";}"
...
export class DiamondFunctionExpression extends FunctionExpression {
    ...
    bareCompile() {
        ...
        if (
            ...
            if (this.getParent().x.static && this.getParent().x.name === null) {
                return [
                    "function" + (hasFun ? " fun_" + $node.antiConflictString : "") + "(){" +
...
export class ParenthesisCallExpression extends $base.Expression {
    ...
    static applyMatch(match, lex) {
        ...
        return new this(
            ...
            new $node.Arr(argumentRanges.map(m =>
                lex.at(m.endIndex) instanceof $lex.Spread ?
                new $node.CallSpread(this.build(lex.part(m.startIndex, m.endIndex - 1))) :
    ...
    static determineUseNew(root, callee, isNew, isNoNew) {
        let useNew = null;
        if (root.hasCompilerDirective("manual new")) {
            ...
            if (isNew) {
            ...
            else if (callee instanceof $node.ClassMeExpression) {
                useNew = true;
            }
            else if (callee instanceof $node.VariableExpression) {
                if (
                    callee.value !== "Number" &&
                    callee.value !== "String" &&
                    callee.value !== "Boolean" &&
                    callee.value !== "Symbol" &&
                    callee.value[0] >= "A" && callee.value[0] <= "Z"
                ) {
                    useNew = true;
                }
                else {
                ...
                callee instanceof $node.DotExpression &&
                callee.y instanceof $node.Piece
            ) {
                if (callee.y.value[0] >= "A" && callee.y.value[0] <= "Z") {
                    useNew = true;
                }
                else {
                    useNew = false;
    ...
    determineUseNew() {
        return this.constructor.determineUseNew(this.getRoot(), this.callee, this.new, this.nonew);
    }

    bareCompile() {
        let args = new J(this.arguments.value.map(m => m.compile()), ",");
        let useNew = this.determineUseNew();
...
export class SpaceCallExpression extends $base.Expression {
    ...
    static applyMatch(match, lex) {
        ...
        return new ParenthesisCallExpression(
            this.build(lex.part(match[0])),
            new $node.Arr([
                lex.at(match[1].endIndex) instanceof $lex.Spread ?
                new $node.CallSpread(this.build(lex.part(match[1].startIndex, match[1].endIndex - 1))) :
                this.build(lex.part(match[1]))
            ]),
            isNew,
            isNonew
