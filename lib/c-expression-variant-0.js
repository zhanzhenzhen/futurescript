...
pool.NormalVariantExpression = class NormalVariantExpression extends pool.Expression {
    ...
    static applyMatch(match, lex) {
        ...
        if (variantName === "ok") {
            return new pool.OkVariantExpression(value);
        }
        else {
            throw new pool.VariantNameError(lex.part(match[1]));
        }
    }
}

pool.OkVariantExpression = class OkVariantExpression extends pool.Expression {
    constructor(x) {
        super();
        this.x = x;
    }
...
pool.FunctionVariantExpression = class FunctionVariantExpression extends pool.Expression {
    constructor(x) {
        super();
        this.x = x;
    }
...
pool.VariantNameError = class VariantNameError extends $lex.SyntaxError {
    constructor(lexPart) {
