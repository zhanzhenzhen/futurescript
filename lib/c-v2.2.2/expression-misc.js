...
export class IfvoidExpression extends $base.BinaryExpression {
    bareCompile() {
        return [
            "(ref_" + $node.antiConflictString + "=", this.x.compile(), ")===undefined?",
            this.y.compile(), ":ref_" + $node.antiConflictString
        ];
    }
}
...
export class IfnullExpression extends $base.BinaryExpression {
    bareCompile() {
        return [
            "(ref_" + $node.antiConflictString + "=", this.x.compile(), ")===undefined||ref_" +
            $node.antiConflictString + "===null?",
            this.y.compile(), ":ref_" + $node.antiConflictString
        ];
    }
}
