...
// Also note that there can't be any async operations before calling `runCodeAsync`.
// For example, `setTimeout(runCodeAsync(code), 0)` can't be used.
// But `await $api.runCodeAsync(code)` can be used if there's no other `await` before.
...
let asyncTimer = setSafeInterval(() => {
    // It means tests are not loaded at the time. If this line doesn't exist,
    // it will mistakenly clear the timer before tests are loaded.
    if (runCodeCursor === 0) return;
