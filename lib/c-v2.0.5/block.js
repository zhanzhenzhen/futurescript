...
`=function(a, b, c, d, e) {
    ...
    return calc(a, b, c) && calc(c, d, e);
            ...
            return new J([
                ...
                libMod, libOk, libIfvoid, libIfnull, libDotDotCalc, libFormattedRegex,
...
export class RootBlock extends ScopeBlock {
    constructor(lex, outputCodeReadability = 0) {return new Promise((resolve, reject) => {(async () => {
    ...
    precompile() {
        this.allNodes.forEach(node => {
            if (node instanceof $node.FunExpression) {
                ...
                let varName = this.replacer();
                this.hoistedCompiled += "var " + varName + ";";
                this.renewReplacer();
                fn.setFunRef(varName);
            }
        });
    }
