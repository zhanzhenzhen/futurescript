...
pool.DotExpression = class DotExpression extends pool.Expression {
    constructor(x, y) {
        super();
        this.x = x;
        this.y = y;
    ...
    rawCompile() {
        let jb = (x, y) => new J(
            y instanceof pool.Expression || y instanceof $node.SymbolMemberName ?
            [x.compile(), "[", y.compile(), "]"] :
            [x.compile(), ".", y.compile()]
        );
        if (this.x instanceof pool.OkVariantExpression) {
...
pool.ObjectExpression = class ObjectExpression extends pool.Expression {
    constructor(value) {
        super();
        this.value = value;
    }
...
pool.ArrayExpression = class ArrayExpression extends pool.Expression {
    constructor(value) {
        super();
        this.value = value;
    }
