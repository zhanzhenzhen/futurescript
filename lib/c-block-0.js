...
export class RootBlock extends ScopeBlock {
    constructor(lex) {
        ...
        this.value.forEach(statement => {
            if (statement instanceof $statement.ImportStatement && statement.batchall) {
                ...
                if (allVariableNames === null) {
                    try {
                        allVariableNames = $lockedApi.fileExports(pathToRead + ".lemo");
                    } catch (ex) {}
                }
                if (allVariableNames === null) {
        ...
        let makePieceLinks = (piece, parent) => {
            ...
            if (piece instanceof $node.LocalVariable) {
                ...
                piece.getParent($expression.ClassExpression).addSymbolMember(piece.value);
            }
        };
        makeLinks(this, null);
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $node.SymbolMemberName) {
                node.getParent($expression.ClassExpression).addNodeToSymbolMember(node, node.value);
            }
        });
        ...
        this.allNodes.forEach(node => {
            ...
            else if (node instanceof $statement.ExportColonStatement) {
    ...
    complyWithJs() {
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $expression.ClassExpression) {
        ...
        this.allNodes.forEach(node => {
            if (
                node instanceof $expression.VariableExpression &&
