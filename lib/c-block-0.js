...
export class RootBlock extends ScopeBlock {
    constructor(lex) {
        ...
        this.value.forEach(statement => {
            if (statement instanceof $statement.ImportStatement && statement.batchall) {
                ...
                let path = lex.path === undefined ? "." : lex.path;
                ...
                let variableNames = $lockedApi.fileExports(pathToRead).filter(m => m !== "default");
    ...
    complyWithJs() {
        ...
        // and raise error.
        if (this.allNodes.some(node =>
            node instanceof $expression.VariableExpression &&
            jsForbiddens.includes(node.value)
        )) {
            throw new Error("Global variable name can't be a JS forbidden identifier name.");
        }
