...
`=function(pattern, flags) {
    ...
    return new RegExp(s, flags);
                ...
                : "";
            return new J([comment, libMod, libOk, libIfvoid, libIfnull, libFormattedRegex, jb]);
        }
        else {
    ...
    constructor(lex) {
        ...
        let makeGeneralLinks = (node, parent) => {
            ...
            if (
                ...
                node.assignees.value.forEach(assignee => {
                    ...
                    else if (assignee instanceof $node.BracketAssignees) {
                        assignee.value.forEach(assignee => {
                            let parent = node.getParent(ScopeBlock);
                            let found = parent.blockWithScopeItem(assignee.variable.value);
                            if (found === null) {
                                parent.addScopeItem(assignee.variable.value);
                ...
                node.arguments.value.forEach(arg => {
                    node.body.addScopeItem(arg.variable.value, scopeItemType.ARGUMENT);
                });
            }
            else if (
                node instanceof $expression.TryExpression &&
                node.catchVar instanceof $node.VariableAssignee
            ) {
                let parent = node.getParent(ScopeBlock);
                let found = parent.blockWithScopeItem(node.catchVar.variable.value);
                if (found === null) {
                    parent.addScopeItem(node.catchVar.variable.value);
                }
            }
            else if (node instanceof $statement.ImportStatement) {
                if (node.catchall !== null) {
                    node.getParent(ScopeBlock)
                    .addScopeItem(node.catchall.variable.value, scopeItemType.IMPORT);
                }
                if (node.mapping !== null) {
                    node.mapping.value.forEach(pair => {
                        node.getParent(ScopeBlock)
                        .addScopeItem(pair.x.variable.value, scopeItemType.IMPORT);
                    });
                }
            }
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $node.VariableAssignee) {
                if (node instanceof $node.ArrowArgument) {
