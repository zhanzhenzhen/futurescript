...
export class Pattern {
    ...
    static matchPattern(tokenTypes, lexPart, leftToRight) {
        ...
        if (!leftToRight) {
            if (
                tokenTypes[0] === Any || tokenTypes[0] === Tokens ||
                tokenTypes[0] instanceof AnyExcept || tokenTypes[0] instanceof TokensExcept
            ) {
                ...
                while (j <= endIndex) {
                    ...
                    if (stack.length === 0) {
                        if (tokenType === undefined) {
                            ...
                            tokenType instanceof AnyExcept || tokenType instanceof TokensExcept
                        ) {
                            if (
                                (tokenType === Any || tokenType === Tokens) &&
                                ...
                                (
                                    ...
                                    token instanceof tokenTypes[tokenTypeIndex + 1] ||
                                    tokenType.tokenTypes.some(m => token instanceof m)
                                )
                            ) { // if the part is zero-length
                                if (tokenType === Any || tokenType instanceof AnyExcept) {
                                    result.push(j);
                                    tokenTypeIndex++;
                                    step = 0;
                                }
                                ...
                                tokenTypeIndex < tokenTypes.length - 1 && (
                                    ...
                                    lex.at(j + 1) instanceof tokenTypes[tokenTypeIndex + 1] ||
                                    tokenType.tokenTypes.some(m => lex.at(j + 1) instanceof m)
                                )
                            ) {
                                if (!inWildcard) {
                                    result.push(j);
