...
export class Pattern {
    ...
    static matchPattern(tokenTypes, lexPart, leftToRight) {
        ...
        for (
            ...
            if (level === 0 && !(
                ...
                let j = i;
                while (j <= endIndex) {
                    let checkToken = (token, ruler) => (
                        ruler.prototype instanceof $lex.Token ? token instanceof ruler : ruler(token, j)
                    );
                    let step = 1;
                    let token = lex.at(j);
                    ...
                    if (stack.length === 0) {
                        if (tokenType === undefined) {
                            ...
                            let checkJumpOut = token => (
                                exceptions.length === 0 ?
                                (
                                    tokenTypeIndex < tokenTypes.length - 1 &&
                                    checkToken(token, tokenTypes[tokenTypeIndex + 1])
                                ) :
                                (
                                    tokenTypeIndex < tokenTypes.length - 1 &&
                                    checkToken(token, tokenTypes[tokenTypeIndex + 1])
                                ) ||
                                exceptions.some(m => token instanceof m)
                            );
                            ...
                            else if (!inWildcard && checkJumpOut(token)) { // if the part is zero-length
                            ...
                            else if (checkJumpOut(lex.at(j + 1))) {
                                if (!inWildcard) {
                                    result.push(j);
                                }
                                inWildcard = false;
                        ...
                        else if (checkToken(token, tokenType)) {
                            result.push(j);
                            tokenTypeIndex++;
                        }
                        else {
    ...
    static matchPatternCapture(tokenTypes, lexPart, leftToRight, capture) {
        ...
        if (match === null) {
            ...
            return capture.map(x => x === null ? null : {
                startIndex: match[x],
