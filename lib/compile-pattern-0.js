        ...
        for (
            ...
            if (level === 0) {
                ...
                if (rightToLeftHead !== null) {
                    result.push(null); // null is a temporary value
                }
                ...
                if (matched) {
                    ...
                    // If first result is null, that is,
                    // it's right-to-left and the first part is wildcard,
                    // then we will replace the temporary placeholder value.
                    if (result[0] === null) {
                        let lastTokenType = tokenTypes[tokenTypes.length - 1];
                        let tokenType =
                            lastTokenType === Any || lastTokenType === Tokens ?
                            tokenTypes[tokenTypes.length - 2] :
                            lastTokenType;
                        let value = 0;
                        for (let j = result[1] - 1; j >= startIndex; j--) {
                            if (lex.at(j) instanceof tokenType) {
                                value = j + 1;
                                break;
                            }
                        }
