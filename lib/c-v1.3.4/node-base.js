...
/* To determine if a chain like `a.b().c'ok.d'ok(e).f` is to be continued. Useful for `'ok` chain
because if we don't store this attribute, then it doesn't know what the chain is, and thus
impossible to compile correctly. For example:
a'ok.b
(a'ok).b
These two are different. Because we don't describe the normal-parenthesis in the tree, we must
have a mark that tells it the chain is ended after `'ok` (for the second one), or ended after `.b`
(for the first one).
*/
export let chainContinues = nextToken => {
    if (
        nextToken instanceof $lex.CallLeftParenthesis ||
        nextToken instanceof $lex.CallLeftBracket ||
        nextToken instanceof $lex.CallLeftBrace ||
