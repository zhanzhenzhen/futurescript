...
export class DotExpression extends $base.Expression {
    ...
    static applyMatch(match, lex) {
        ...
        let ok = false;
        if (
            lex.at(match[0].endIndex - 1) instanceof $lex.NormalVariant &&
            lex.at(match[0].endIndex) instanceof $lex.NormalToken &&
            lex.at(match[0].endIndex).value === "ok"
        ) {
            left = this.build(lex.part(match[0].startIndex, match[0].endIndex - 2));
            ok = true;
        }
        else {
            left = this.build(lex.part(match[0]));
        }
