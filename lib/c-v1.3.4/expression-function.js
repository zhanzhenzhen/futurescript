...
export class ArrowFunctionExpression extends FunctionExpression {
    ...
    bareCompile() {
        ...
        let simplifiedVariables = new J(args.map(m => m.variable.compile()), ",");
        if (!this.argumentsReal) {
            variables = new J(["[", variables, "]"]);
            simplifiedVariables = new J(["[", simplifiedVariables, "]"]);
        }
        ...
        if (
            ...
            if (this.getParent($node.ClassExpression) === null) {
                ...
                return [
                    "(", variables, ")=>{var ", name,
                    ";return (", name, "=(", simplifiedVariables, ")=>{",
                    voidDefaults, nullDefaults,
                    "return ", this.body.compile(), ";})(", simplifiedVariables, ");}"
                ];
            }
        }
    }
...
export class ParenthesisCallExpression extends $base.Expression {
    constructor(callee, args, isNew = false, isNonew = false, chainContinues = false) {
        super();
        this.callee = callee;
        this.arguments = args;
        this.new = isNew;
    ...
    bareCompile() {
        let jb = (callee) => {
            ...
            if (callee instanceof $node.DotDotExpression) {
