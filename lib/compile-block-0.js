    ...
    constructor(lex) {
        ...
        let makeGeneralLinks = (node, parent) => {
            ...
            if (
                ...
                let variableScopeBlock = scopeBlock.blockWithScopeItem(node.assignee.variable);
                if (variableScopeBlock === null) {
                    scopeBlock.addScopeItem(node.assignee.variable);
                }
            }
                ...
                node.assignees.value.forEach(assignee => {
                    if (assignee instanceof $node.VariableAssignee) {
                        let scopeBlock = node.getParent(ScopeBlock);
                        let variableScopeBlock = scopeBlock.blockWithScopeItem(assignee.variable);
                        if (variableScopeBlock === null) {
                            scopeBlock.addScopeItem(assignee.variable);
                        }
                    }
                });
            }
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $node.VariableAssignee) {
                let scopeBlock = node.getParent(ScopeBlock);
                let variableScopeBlock = scopeBlock.blockWithScopeItem(node.variable);
                if (variableScopeBlock !== null) {
                    variableScopeBlock.addNodeToScopeItem(node, node.variable);
                }
            }
            else if (node instanceof $expression.VariableExpression) {
                let scopeBlock = node.getParent(ScopeBlock);
                let variableScopeBlock = scopeBlock.blockWithScopeItem(node.value);
                if (variableScopeBlock !== null) {
                    variableScopeBlock.addNodeToScopeItem(node, node.value);
                }
            }
