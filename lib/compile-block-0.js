...
export class RootBlock extends ScopeBlock {
    constructor(lex) {
        ...
        let makeLinks = (thing, parent) => {
            if (thing === true || thing === false || thing === null || thing === undefined) {
            }
            else if (thing instanceof $node.AtomNode) {
        ...
        let makeGeneralLinks = (node, parent) => {
            ...
            if (
                ...
                node.getParent(ScopeBlock).addAssignmentVariable(node.assignee.variable.value);
            }
                ...
                node.assignees.value.forEach(assignee => {
                    if (assignee instanceof $node.VariableAssignee) {
                        node.getParent(ScopeBlock).addAssignmentVariable(assignee.variable.value);
                    }
                });
            }
