...
export class ScopeBlock extends Block {
    ...
    addScopeVariable(variable) {
        if (!this.hasScopeVariable(variable)) {
            this._scopeVariables[variable] = [];
        }
    }

    hasScopeVariable(variable) {
        return Array.isArray(this._scopeVariables[variable]);
    }

    blockWithScopeVariable(variable) {
        let next = this;
        while (next !== null) {
    ...
    constructor(lex) {
        ...
        let makeGeneralLinks = (node, parent) => {
            ...
            if (
                ...
                let variableScopeBlock = scopeBlock.blockWithScopeVariable(node.assignee.variable);
                if (variableScopeBlock === null) {
                ...
                node.assignees.value.forEach(assignee => {
                    if (assignee instanceof $node.VariableAssignee) {
                        let scopeBlock = node.getParent(ScopeBlock);
                        let variableScopeBlock = scopeBlock.blockWithScopeVariable(assignee.variable);
                        if (variableScopeBlock === null) {
        ...
        this.allNodes.forEach(node => {
            if (node instanceof $node.VariableAssignee) {
                let scopeBlock = node.getParent(ScopeBlock);
                let variableScopeBlock = scopeBlock.blockWithScopeVariable(node.variable);
                if (variableScopeBlock !== null) {
                    variableScopeBlock.getScopeVariables()[node.variable].push(node);
                }
            }
            else if (node instanceof $expression.VariableExpression) {
                let scopeBlock = node.getParent(ScopeBlock);
                let variableScopeBlock = scopeBlock.blockWithScopeVariable(node.value);
                if (variableScopeBlock !== null) {
                    variableScopeBlock.getScopeVariables()[node.value].push(node);
                }
            }
            else if (node instanceof $node.ArrowArgument) {
                node.getParent($expression.ArrowFunctionExpression)
                .body.getScopeVariables()[node.variable].push(node);
            }
        });
    }
