...
export class RootBlock extends Block {
    constructor(lex) {
        ...
        let makeLinks = (thing, parent) => {
            if (thing instanceof $statement.Atom) {
            ...
            else if (thing instanceof $statement.Arr) {
                makeArrLinks(thing, parent);
            }
            else if (thing instanceof $statement.NameValue) {
                makeNameValueLinks(thing, parent);
            }
        };
        let makeBlockLinks = (block, parent) => {
            block.value.forEach(statement => {
                ...
                statement.helper = {};
                statement.helper.parent = block;
            });
            block.helper = {};
            block.helper.parent = parent;
        };
        let makeExpressionLinks = (expression, parent) => {
            ...
            expression.helper = {};
            expression.helper.parent = parent;
        };
        let makeArrLinks = (arr, parent) => {
            ...
            arr.helper = {};
            arr.helper.parent = parent;
        };
        let makeNameValueLinks = (nv, parent) => {
            makeLinks(nv.name, nv);
            makeLinks(nv.value, nv);
            nv.helper = {};
            nv.helper.parent = parent;
        };
        let makeAtomLinks = (atom, parent) => {
            atom.helper = {};
            atom.helper.parent = parent;
        };
