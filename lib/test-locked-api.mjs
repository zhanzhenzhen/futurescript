...
export let currentVersion = packageInfo.version;

export let tempDirectory = "target/temp";
...
export let writeTextFile = function(path, text, encoding = TextEncoding.utf8) {
    ...
    if (encoding === TextEncoding.utf8) {
        ...
        let rawBytes = Buffer.from(text, "utf8");
        bytes = Buffer.alloc(rawBytes.length + 3);
        bytes[0] = 0xEF;
        bytes[1] = 0xBB;
        bytes[2] = 0xBF;
        ...
        let rawBytes = Buffer.from(text, "utf16le");
        bytes = Buffer.alloc(rawBytes.length + 2);
        bytes[0] = 0xFF;
        bytes[1] = 0xFE;
