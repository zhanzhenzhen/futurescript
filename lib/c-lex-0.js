...
export class SyntaxError extends $lockedApi.SyntaxError {
    constructor(lexPart, message) {
        ...
        let startIndex = null;
        let endIndex = null;
        for (let i = lexPart.startIndex; i >= 0; i--) {
            let t = lex.at(i).rawStartIndex;
            if (t !== undefined) {
                startIndex = t;
                break;
            }
        }
        for (let i = lexPart.endIndex; i < lex.count(); i++) {
            let t = lex.at(i).rawEndIndex;
            if (t !== undefined) {
                endIndex = t;
                break;
            }
