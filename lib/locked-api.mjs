...
export let readTextFile = async function(path) {
    path = slashPath(path);
    if ($env.isNode) {
        // Use dynamic import to prevent browser from importing.
        let $util = (await import("util")).default;
        let $fs = (await import("fs")).default;
        let $path = (await import("path")).default.posix;
...
export let fileExports = async function(path) {
    let output = await generateOutput({path: path, level: OutputLevel.exports});
...
*/
export let generateOutput = async function(input) {
    ...
    if (adjustedInput.code === undefined) {
        adjustedInput.code = await readTextFile(adjustedInput.path);
    }
    ...
    let mainModule = null;
    try {
        mainModule = await import("./c-v" + version + "/main.mjs");
    }
    catch (ex) {
