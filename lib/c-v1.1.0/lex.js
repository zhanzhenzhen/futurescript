        ...
        this.value.forEach((token, index) => {
            if (
                ...
                if (token instanceof LeftChevron) {
                    ...
                    let container = {type: Brace, tokens: []};
                    stack.push(container);
                    this._braceContainers.push(container);
                }
                stack[stack.length - 1].leftIndex = index;
                newValue.push(token);
            }
            else if (
                token instanceof RightChevron ||
                ...
                stack[stack.length - 1].rightIndex = index;
                level--;
                stack.pop();
                token.level = level;
                token.container = stack[stack.length - 1];
    ...
    _round24() {
        this._braceContainers.forEach(container => {
            if (
                ...
                !(this.value[container.leftIndex + 1] instanceof Colon)
            ) {
                if (
                    !container.tokens.some(m => m instanceof Comma || m instanceof Colon) &&
                    container.tokens.length >= 2
                ) {
                    ...
                    let endIndex = functionPos === -1 ? container.tokens.length - 1 : functionPos;
                    let isKey = false;
                    for (let i = endIndex; i >= 1; i--) {
                        if (
                            !(container.tokens[i] instanceof ArrowFunction) &&
                            (
                                container.tokens[i] instanceof NormalToken ||
                                container.tokens[i] instanceof Keyword ||
                                container.tokens[i - 1] instanceof NormalToken ||
                                container.tokens[i - 1] instanceof Keyword
                            )
                        ) {
                            if (isKey) {
                                ...
                                newToken.next = container.tokens[i];
                                if (container.tokens[i - 1] instanceof Keyword) {
                                    let normalToken = container.tokens[i - 1].getOriginalTokens()[0];
                                    normalToken.next = newToken;
                                    if (i >= 2) {
                                        container.tokens[i - 2].next = normalToken;
                                    }
                                    else {
                    ...
                    container.tokens.concat([this.value[container.rightIndex]]).forEach((token, index) => {
                        if (token instanceof Comma || token instanceof RightBrace) {
                            if (!hasColon) {
